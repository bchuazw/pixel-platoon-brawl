    ...
    <div className="pointer-events-auto absolute left-0 sm:left-2 bottom-10 sm:bottom-12 w-44 sm:w-56 z-20">
      <div className="glass-panel rounded-xl p-3 relative overflow-hidden">
        <div className="absolute inset-0 z-10 bg-background/50 backdrop-blur-[1px] flex flex-col items-center justify-center rounded-xl">
          ...
          <div className="text-[11px] font-display font-bold text-accent/50 tracking-[0.15em]">COMING SOON</div>
        </div>

        <div className="opacity-40">
          <div className="flex items-center gap-2 mb-2">
            <Gift className="w-3.5 h-3.5 text-accent" />
            <span className="text-[12px] font-display font-bold text-accent tracking-[0.15em]">SPONSOR</span>
          </div>
          <p className="text-[10px] text-muted-foreground mb-2">Send gifts to your favorite combatant</p>
          <div className="space-y-1.5">
            {['🔫 Weapon Drop', '❤️ Med Kit', '🛡️ Armor Vest'].map(item => (
              <div key={item} className="flex items-center justify-between px-2 py-1.5 bg-secondary/30 rounded border border-border/10">
                ...
                <span className="text-[10px] text-accent/50">⭐2</span>
              </div>
            ))}
          </div>
        </div>
