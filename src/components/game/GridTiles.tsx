...
function Tile({ tile, isMovable, isAttackable, isOutOfZone, onClick }: {
  ...
  return (
    <mesh
      ...
      onPointerOver={() => { if (ref.current) (ref.current.material as THREE.MeshStandardMaterial).emissiveIntensity = emissiveIntensity + 0.15; }}
      ...
      return (
        <group position={[tile.x, baseY, tile.z]}>
          <mesh position={[0, 0.22, 0]}>
            <cylinderGeometry args={[0.2, 0.22, 0.45, 8]} />
            <meshStandardMaterial color="#5a6a5a" roughness={0.7} metalness={0.3} />
          </mesh>
          <mesh position={[0, 0.44, 0]}>
            <cylinderGeometry args={[0.19, 0.19, 0.03, 8]} />
            <meshStandardMaterial color="#4a5a4a" metalness={0.5} />
          </mesh>
        </group>
      );
    case 'sandbag':
      return (
        <group position={[tile.x, baseY, tile.z]}>
          <mesh position={[0, 0.1, 0]}>
            ...
            <meshStandardMaterial color="#b0a070" roughness={1} />
          </mesh>
          <mesh position={[0, 0.22, 0]}>
      ...
      return (
        <group position={[tile.x, baseY, tile.z]}>
          <mesh position={[0, 0.15, 0]} rotation={[0.1, tile.variant * 0.8, 0]}>
            ...
            <meshStandardMaterial color="#6a6a6e" roughness={0.95} />
          </mesh>
          <mesh position={[0.15, 0.08, 0.12]} rotation={[0.2, 1.2, 0]}>
      ...
      return (
        <group position={[tile.x, baseY, tile.z]}>
          ...
          <mesh position={[0.1, 0.12, 0.1]}>
            ...
            <meshStandardMaterial color="#2e6e22" roughness={0.9} />
          </mesh>
        </group>
      );
    case 'tree':
...
export function GridTiles({ grid, movableTiles, attackableTiles, shrinkLevel, onTileClick }: GridTilesProps) {
  // Pre-compute grass positions for decoration
